...
import { X, Save, Trash2, Activity, Package, Bot, ClipboardList, Plus, Users, ImageIcon, Monitor } from 'lucide-react';
...
type TabType = 'overview' | 'planning' | 'team' | 'activity' | 'deliverables' | 'images' | 'sessions' | 'terminal';
...
export function TaskModal({ task, onClose, workspaceId }: TaskModalProps) {
  ...
  const tabs = [
    ...
    { id: 'terminal' as TabType, label: 'Terminal', icon: <Monitor className="w-4 h-4" /> },
  ];
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-mc-bg-secondary border border-mc-border rounded-t-xl sm:rounded-lg w-full max-w-2xl max-h-[92vh] sm:max-h-[90vh] flex flex-col pb-[env(safe-area-inset-bottom)] sm:pb-0">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {/* Terminal Tab */}
          {activeTab === 'terminal' && task && (
            <TerminalTab taskId={task.id} />
          )}
        </div>
