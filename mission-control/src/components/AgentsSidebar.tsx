...
import { Plus, ChevronRight, ChevronLeft, Zap, ZapOff, Loader2, Search, Terminal } from 'lucide-react';
...
export function AgentsSidebar({ workspaceId, mobileMode = false, isPortrait = true }: AgentsSidebarProps) {
  ...
  const handleViewTerminal = async (agent: Agent, e: React.MouseEvent) => {
    e.stopPropagation();
    // Find the launcher session ID for this agent
    try {
      const res = await fetch(`/api/agents/${agent.id}/openclaw`);
      if (res.ok) {
        const data = await res.json();
        const sessionId = data.session?.openclaw_session_id?.replace('launcher:', '') || null;
        if (sessionId && window.parent !== window) {
          window.parent.postMessage(
            { type: 'open-terminal', sessionId, agentName: agent.name },
            '*'
          );
        } else if (sessionId) {
          // Not in iframe, open launcher directly
          window.open(`http://localhost:3002/#active`, '_blank');
        }
      }
    } catch (err) {
      console.error('Failed to get session for terminal:', err);
    }
  };
            ...
            <div key={agent.id} className={`w-full rounded hover:bg-mc-bg-tertiary transition-colors ${selectedAgent?.id === agent.id ? 'bg-mc-bg-tertiary' : ''}`}>
              ...
              {agent.status === 'working' && (
                <div className="px-2 pb-2">
                  <button
                    onClick={(e) => handleViewTerminal(agent, e)}
                    className="w-full min-h-9 flex items-center justify-center gap-2 px-2 rounded text-xs transition-colors bg-mc-accent/10 text-mc-accent hover:bg-mc-accent/20 border border-mc-accent/20"
                  >
